                    ...
                    "containers": [
                        {
                            ...
                            "volumeMounts": [{"mountPath": "/opt", "name": "config"}],
                        }
                    ],
                    "resources": {
                    ...
                    "volumes": [
                        {
                            ...
                            "configMap": {
                                ...
                                "items": [
                                    {"key": "sample_code.py", "path": "sample_code.py"}
                                ],
                            },
                        }
                    ],
...
def main():
    ...
    try:
        my_kube_ray_api.core_v1_api.create_namespaced_config_map(
            "default", configmap_body
        )

    except ApiException as e:
        if e.status == 409:
            print(
                "configmap {} already exists = {} moving on...".format(
                    configmap_body["metadata"]["name"], e
                )
            )
        else:
    ...
    time.sleep(3)
    try:
        pod_list: client.V1PodList = my_kube_ray_api.core_v1_api.list_namespaced_pod(
            namespace="default",
        ...
        if pod_list is not None:
            for pod in pod_list.items:
                try:
                    ...
                    exec_command = ["python", "/opt/sample_code.py"]

                    print(
                        "executing a Python command in the raycluster: {}".format(
                            exec_command
                        )
                    )
                    # executing a ray command in the head pod
                    resp = stream(
                        my_kube_ray_api.core_v1_api.connect_get_namespaced_pod_exec,
                        pod.metadata.name,
                        "default",
                    ...
                    time.sleep(3)
                    print(
                        "getting the logs from the raycluster pod: {}".format(
                            pod.metadata.name
                        )
                    )
                    api_response = my_kube_ray_api.core_v1_api.read_namespaced_pod_log(
    ...
    kube_ray_list = my_kube_ray_api.list_ray_clusters(
    ...
    try:
        my_kube_ray_api.core_v1_api.delete_namespaced_config_map(
            configmap_body["metadata"]["name"], "default"
        )  # deleting the configmap
        print("deleting configmap: {}".format(configmap_body["metadata"]["name"]))
    except ApiException as e:
        if e.status == 404:
            print(
                "configmap  = {}, does not exist moving on...".format(
                    configmap_body["metadata"]["name"], e
                )
            )
        else:
