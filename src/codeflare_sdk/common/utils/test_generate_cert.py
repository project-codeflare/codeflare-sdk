...
def test_client_cert_aki_matches_ca_ski(mocker):
    """
    Verify the client certificate's AuthorityKeyIdentifier copies the CA's
    actual SubjectKeyIdentifier, rather than recomputing it from the public key.
    """
    ...
    mocker.patch(
        "codeflare_sdk.common.utils.generate_cert.get_secret_name",
        return_value="ca-secret-aki-test",
    )
    mocker.patch(
        "kubernetes.client.CoreV1Api.read_namespaced_secret",
        side_effect=secret_ca_retreival_generic,
    ...
    generate_tls_cert("aki-test", "default", force_regenerate=True)
    tls_dir = os.path.join(os.getcwd(), "aki-test-default")

    with open(os.path.join(tls_dir, "tls.crt"), "rb") as f:
        tls_cert = load_pem_x509_certificate(f.read())
    ...
    client_aki = tls_cert.extensions.get_extension_for_class(
        cx509.AuthorityKeyIdentifier
    ).value

    assert client_aki.key_identifier == ca_ski.key_identifier, (
        f"Client AKI {client_aki.key_identifier.hex()} must match "
        f"CA SKI {ca_ski.key_identifier.hex()}"
    )
...
def _build_ca_with_custom_ski(custom_ski_bytes):
    """Helper: create a CA cert whose SKI is set to an arbitrary value,
    simulating a CA generated by a different library (e.g. Go's crypto/x509)."""
    from cryptography import x509 as cx509
    ...
    builder = (
        ...
        .subject_name(cx509.Name([cx509.NameAttribute(NameOID.COMMON_NAME, "test-ca")]))
        ...
        .add_extension(cx509.SubjectKeyIdentifier(custom_ski_bytes), critical=False)
    )
    ca_cert = builder.sign(private_key, hashes.SHA256())
    return private_key, ca_cert


def test_client_cert_aki_matches_cross_library_ca_ski(mocker):
    """
    Simulate a CA generated by Go's crypto/x509 which computes a different SKI
    than Python's cryptography library would.  The client cert's AKI must copy
    the CA's actual SKI verbatim, NOT recompute it from the public key.
    """
    from cryptography import x509 as cx509

    fake_go_ski = (
        b"\xf3\x3f\x71\x69\x34\x38\xe7\x0c\x4f\x76"
        b"\xd8\x9b\xbb\x64\xea\x9d\xd5\x01\xd6\x08"
    )

    ca_key, ca_cert = _build_ca_with_custom_ski(fake_go_ski)

    ca_cert_b64 = base64.b64encode(ca_cert.public_bytes(Encoding.PEM)).decode("utf-8")
    ca_key_b64 = base64.b64encode(
    ...
    def mock_secret_retrieval(secret_name, namespace):
        return client.models.V1Secret(
            data={"ca.crt": ca_cert_b64, "ca.key": ca_key_b64}
        )

    mocker.patch("kubernetes.config.load_kube_config", return_value="ignore")
    mocker.patch(
        "codeflare_sdk.common.utils.generate_cert.get_secret_name",
        return_value="ca-secret-cross-lib",
    )
    mocker.patch(
        "kubernetes.client.CoreV1Api.read_namespaced_secret",
        side_effect=mock_secret_retrieval,
    )
    mocker.patch(
        "codeflare_sdk.common.utils.generate_cert._get_tls_base_dir",
        return_value=Path(os.getcwd()),
    ...
    generate_tls_cert("cross-lib", "default", force_regenerate=True)
    tls_dir = os.path.join(os.getcwd(), "cross-lib-default")
    ...
    python_ski = cx509.SubjectKeyIdentifier.from_public_key(
        ca_cert.public_key()
    ).key_identifier

    assert client_aki.key_identifier == fake_go_ski, (
        f"AKI must match the CA's actual SKI ({fake_go_ski.hex()}), "
        f"got {client_aki.key_identifier.hex()}"
    )
    assert client_aki.key_identifier != python_ski, (
        "AKI must NOT be the recomputed value from from_issuer_public_key()"
    )
...
def test_client_cert_aki_fallback_when_ca_has_no_ski(mocker):
    """
    When the CA certificate lacks a SubjectKeyIdentifier extension,
    the code should fall back to from_issuer_public_key() without error.
    """
    from cryptography import x509 as cx509
    from cryptography.hazmat.primitives.asymmetric import rsa
    from cryptography.hazmat.primitives import hashes, serialization
    ...
    ca_cert = (
        cx509.CertificateBuilder()
        .subject_name(
            cx509.Name([cx509.NameAttribute(NameOID.COMMON_NAME, "no-ski-ca")])
        )
        .issuer_name(
            cx509.Name([cx509.NameAttribute(NameOID.COMMON_NAME, "no-ski-ca")])
        )
    ...
    generate_tls_cert("no-ski", "default", force_regenerate=True)
    tls_dir = os.path.join(os.getcwd(), "no-ski-default")
    ...
    expected_aki = cx509.AuthorityKeyIdentifier.from_issuer_public_key(
        ca_cert.public_key()
    )

    assert client_aki.key_identifier == expected_aki.key_identifier, (
        "When CA has no SKI, AKI should fall back to from_issuer_public_key()"
    )
