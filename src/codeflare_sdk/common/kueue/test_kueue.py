...
def test_none_local_queue(mocker):
    ...
    cluster = RayCluster(name="unit-test-aw-kueue", namespace="ns", local_queue=None)
...
def test_cleanup():
    if os.path.exists(f"{aw_dir}unit-test-cluster-kueue.yaml"):
        os.remove(f"{aw_dir}unit-test-cluster-kueue.yaml")
    if os.path.exists(f"{aw_dir}unit-test-aw-kueue.yaml"):
